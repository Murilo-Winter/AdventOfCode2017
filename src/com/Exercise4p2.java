...
public class Exercise4p2 {
    ...
    String[][] passwordMatrix = new String[rows][columns];
    ...
    public void execute() {
        ...
        for(int r = 0; r < rows; r++){
            ...
            for(int c=0; c < columns; c++){
                if (passwordMatrix[r][c].equals("0")) break;
                for(int nextColumn=c+1; nextColumn<columns; nextColumn++){
                    if(!checkValid(passwordMatrix[r][c], passwordMatrix[r][nextColumn])) {
                        validLine = false;
                        break;
                    }
                }
            ...
            if (validLine) validLines++;
        }

        System.out.print(validLines);
    }

    private void initializeMatrix(){
        for(int c = 0; c < columns; c++){
            for(int r = 0; r < rows; r++) {
                passwordMatrix[r][c] = "0";
            }
        }
    }
    ...
    private void buildMatrix() {
        ...
        for(int i = 0; i < input.length(); i++) {
            ...
            if (currentCharacter == ' ' ){
                passwordMatrix[row][column] = currentPassword.toString();
            ...
            currentPassword.append(currentCharacter);

        }
    }

    private boolean checkValid(String valueA, String valueB){
        if (valueA.length() != valueB.length())
            return true;

        int lenght = valueA.length();

        char[] valueACharacters = valueA.toCharArray();
        char[] valueBCharacters = valueB.toCharArray();

        for(int i=0; i < lenght; i++){

            char character = valueACharacters[i];

            for(int j=0; j<valueBCharacters.length; j++){
                if(valueBCharacters[j] == character){
                    valueBCharacters[j] = '0';
                    break;
                }
            }
