...
public class Exercise2p2 {
    ...
    public Exercise2p2() {
        rows = 16;
        columns = 16;
        this.spreadsheet = new int[rows][columns];
    ...
    private void buildSpreadsheet() {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            if (currentCharacter == ' ' || currentCharacter == '\t') {
                spreadsheet[column][row] = Integer.valueOf(currentNumber.toString());
        ...
        for (int r = 0; r < rows; r++) {
            ...
            for (int c = 0; c < columns; c++) {
                for (int nextColumn = c + 1; nextColumn < columns; nextColumn++) {
                    if (spreadsheet[r][c] % spreadsheet[r][nextColumn] == 0) {
                        checksum += spreadsheet[r][c] / spreadsheet[r][nextColumn];
                        elementFound = true;
                    }
                    if (spreadsheet[r][nextColumn] % spreadsheet[r][c] == 0) {
                        checksum += spreadsheet[r][nextColumn] / spreadsheet[r][c];
                        elementFound = true;
                    }
