...
public class Exercise12p2 {
    ...
    Map<Integer, List<Integer>> programsHash = new HashMap<>();

    Map<Integer, List<Integer>> visitedPrograms = new HashMap<>();
    ...
    public void execute() {
        ...
        for(int i = 0; i < 2000; i++) {
            ...
            if(!groupAlreadyVisited(i))
                formGroup(i, i, programsHash.get(i));
        }

        System.out.print(visitedPrograms.size());

    }

    private void buildProgramsHash() {
        for (String line : inputList) {
            List<String> brokenLine = Arrays.asList(line.split(" <-> "));

            Integer programNumber = Integer.valueOf(brokenLine.get(0));

            String sublist = brokenLine.get(1);
            List<String> accessListStr = Arrays.asList(sublist.split(", "));

            List<Integer> accessList = new ArrayList<>();
            for (String accessString : accessListStr) {
                accessList.add(Integer.valueOf(accessString));
            }

            programsHash.put(programNumber, accessList);
        }
    }

    private boolean groupAlreadyVisited(Integer refProgram) {

        for (Map.Entry<Integer, List<Integer>> entry : visitedPrograms.entrySet()) {

            if (entry.getKey().equals(refProgram) || entry.getValue().contains(refProgram)) {
                return true;
            }
        }
        return false;
    ...
    private void formGroup(Integer groupKey, Integer program, List<Integer> connections) {

        //Insert group key if still doesn't exist
        if(!visitedPrograms.containsKey(groupKey))
            visitedPrograms.put(groupKey, new ArrayList<>());

        if (!visitedPrograms.get(groupKey).contains(program)){
            visitedPrograms.get(groupKey).add(program);
        }

        for (Integer connection : connections) {
            if (!visitedPrograms.get(groupKey).contains(connection))
                formGroup(groupKey, connection, programsHash.get(connection));
        }
    }
