...
public class Exercise24p2 {
	...
	public void execute() {
		...
		buildBridge(0, components, 0, 0);

		System.out.println(highestStrenght);
	}

	private void buildBridge(int pin, List<Component> components, int strength, int length) {
		...
		for(int i = 0; i < components.size(); i++) {
			...
			if(component.a == pin || component.b == pin) {
				...
				if(newLength > highestLength) {
					highestLength = newLength;
					highestStrenght = newStrength;
				}
				else if (newLength == highestLength && newStrength > highestStrenght){
					highestLength = newLength;
					highestStrenght = newStrength;
				}

				buildBridge(newPin, remainingComponents, newStrength, newLength);
			}
		}
	}
