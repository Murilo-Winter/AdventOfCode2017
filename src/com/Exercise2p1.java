...
public class Exercise2p1 {
    ...
    private int calcCheckSum() {
        ...
        int lowestRowNumber = 999999999;
        int checksum = 0;

        for( int r = 0; r < rows; r++) {
            for (int c = 0; c < columns; c++) {
                if (spreadsheet[r][c] < lowestRowNumber)
                    lowestRowNumber = spreadsheet[r][c];

                if (spreadsheet[r][c] > biggestRowNumber)
                    biggestRowNumber = spreadsheet[r][c];

                if (c == columns-1) {
                    checksum += biggestRowNumber - lowestRowNumber;
                    lowestRowNumber = 999999999;
                    biggestRowNumber = 0;
                }
            }
        }
