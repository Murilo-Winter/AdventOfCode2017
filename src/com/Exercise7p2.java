...
//                "cntj (57)";
        ...
        setChildren();
        setParent();
        Program parent = findParentless();

        calculateSummedWeight(parent);
        findUnbalanced(parent);
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            switch (action){
                case 'n':
                    ...
                    if(currentCharacter == ' '){
                        program = new Program();
                        program.name = currentElement.toString();
                ...
                case 'w':
                    ...
                    if(currentCharacter == ')'){
                        program.weight = Integer.valueOf(currentElement.toString());
                        currentElement.delete(0, currentElement.length());
                        i++;
                ...
                case 'p':
                    ...
                    if(i == input.length() || currentCharacter == '\n'){
                        program.childrenName.add(currentElement.toString());
                        currentElement.delete(0, currentElement.length());
                        programList.add(program);
                        action = 'n';
                        continue;
    ...
    private void setChildren(){
        ...
        for(int i = 0; i < programList.size(); i++){
            ...
            if(currentProgram.hasChildren()){
                //For each children it contains
                for(int j = 0; j < currentProgram.childrenName.size(); j++){
                    String currentChildrenName = currentProgram.childrenName.get(j);

                    //Iterate remaining programs to find the correct one
                    for(int k = 0; k < programList.size(); k++){
                        if(currentChildrenName.equals(programList.get(k).name)) {
                            currentProgram.children.add(programList.get(k));
                            break;
                        }
                    }
                }
    ...
    private void setParent() {
        for (int i = 0; i < programList.size(); i++) {
            for( int j = 0; j < programList.get(i).children.size(); j++){
                programList.get(i).children.get(j).parent= programList.get(i);
            }
        }
    }

    private Program findParentless(){
    ...
    private void calculateSummedWeight(Program element){
        element.totalWeight = element.weight;

        for(Program child : element.children){
            if (child.totalWeight == 0)
            {
                calculateSummedWeight(child);
            }
            element.totalWeight += child.totalWeight;
        }

    }

    private void findUnbalanced(Program element){

        Program childA = new Program();
        Program childB = new Program();
        Program childC = new Program();
        Program oddChild = new Program();

        for(int i=0; i< element.children.size()-2; i++){
            childA = element.children.get(i);
            childB = element.children.get(i+1);
            childC = element.children.get(i+2);

            if(childA.totalWeight == childB.totalWeight && childB.totalWeight == childC.totalWeight){
                break;
            } else if (childA.totalWeight == childB.totalWeight) {
                oddChild = childC;
                break;
            } else if (childA.totalWeight == childC.totalWeight) {
                oddChild = childB;
                break;
            } else if (childB.totalWeight == childC.totalWeight) {
                oddChild = childA;
                break;
            }
        }

        if(oddChild.totalWeight != 0) {
            System.out.print(childA.totalWeight);
            System.out.print("\n");
            System.out.print(childA.weight);
            System.out.print("\n");
            System.out.print(childB.totalWeight);
            System.out.print("\n");
            System.out.print(childB.weight);
            System.out.print("\n");
            System.out.print(childC.totalWeight);
            System.out.print("\n");
            System.out.print(childC.weight);
            System.out.print("\n");
            System.out.print("\n");
        }

        for(Program child : element.children){
            findUnbalanced(child);
        }
